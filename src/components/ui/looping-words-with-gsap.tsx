...
export const LoopingWords: React.FC<LoopingWordsProps> = ({ words, className = '', hideCredits = false }) => {
  const wordListRef = useRef<HTMLUListElement>(null);
  const edgeElementRef = useRef<HTMLDivElement>(null);
  const timelineRef = useRef<gsap.core.Timeline | null>(null);

  // Ensure enough items exist for the continuous GSAP DOM shift and wrap-around logic
  const displayWords = React.useMemo(() => {
    if (!words || words.length === 0) return ['राधा', 'राधा', 'राधा', 'राधा', 'राधा'];
    if (words.length < 5) {
  ...
  const updateEdgeWidth = useCallback(() => {
    ...
    if (centerWord) {
      const targetElement = (centerWord.querySelector('.looping-words__p') as HTMLElement) || centerWord;
      const centerWordWidth = targetElement.getBoundingClientRect().width;
      const listWidth = wordList.getBoundingClientRect().width;
      const percentageWidth = Math.min(100, Math.max(10, ((centerWordWidth + 24) / listWidth) * 100));
  ...
  return (
    <>
      <div className={`cloneable ${className}`}>
